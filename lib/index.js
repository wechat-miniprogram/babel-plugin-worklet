...
// is `<name>.__boundThis = ...` already the statement right after the declaration?
function hasBoundThisAnnotation(t, declaration, name) {
  const next = declaration.getNextSibling()
  if (!next || !next.isExpressionStatement()) {
    return false
  }
  ...
  return (
    ...
    !expr.left.computed &&
    t.isIdentifier(expr.left.object, { name }) &&
    t.isIdentifier(expr.left.property, { name: '__boundThis' })
  )
}

// `const bump = this.bumpCount.bind(this)` -> record the bound receiver
function annotateBoundThis(t, fun, closure) {
  closure.forEach((_node, name) => {
    ...
    if (
      ...
      !t.isIdentifier(init.callee.property, { name: 'bind' }) ||
      init.arguments.length === 0 ||
      t.isSpreadElement(init.arguments[0])
    ) {
      return
    }
    const declaration = binding.path.parentPath
    if (!declaration.isVariableDeclaration() || !t.isIdentifier(declarator.id)) {
      return
    }
    const boundName = declarator.id.name
    if (hasBoundThisAnnotation(t, declaration, boundName)) {
      return
    }
    declaration.insertAfter(
      t.expressionStatement(
        t.assignmentExpression(
          '=',
          t.memberExpression(t.identifier(boundName), t.identifier('__boundThis')),
          t.cloneNode(init.arguments[0]),
        ),
      ),
    )
  })
...
function extractWorkletData(t, fun, fileName) {
  // Extract worklet data: closure, variables, funString, workletHash, locationFileName
  // This is the common logic shared by makeWorklet and buildWorkletProperties
  ...
  annotateBoundThis(t, fun, closure)
  ...
  if (loc) {
    ...
    if (typeof line === 'number' && typeof column === 'number') {
      locationFileName = `${fileName} (${line}:${column})`
    }
  }
...
function makeWorklet(t, fun, fileName) {
  ...
  const { variables, closure, closureGenerator, funString, workletHash, locationFileName } = extractWorkletData(
    t,
    fun,
    fileName,
  )
  ...
  const statements = [
    ...
    t.expressionStatement(
      t.assignmentExpression(
        ...
        t.stringLiteral(locationFileName),
      ),
    ),
    t.expressionStatement(
      t.assignmentExpression(
...
function buildWorkletProperties(t, fun, fileName) {
  // Build the worklet properties object for Object.assign
  ...
  return t.objectExpression([
    t.objectProperty(t.identifier('_closure'), closureGenerator.generate(t, variables, closure.keys())),
    t.objectProperty(t.identifier('asString'), t.stringLiteral(funString)),
    t.objectProperty(t.identifier('__workletHash'), t.numericLiteral(workletHash)),
    t.objectProperty(t.identifier('__location'), t.stringLiteral(locationFileName)),
    t.objectProperty(t.identifier('__worklet'), t.booleanLiteral(true)),
  ])
}
...
function processWorkletFunction(t, fun, fileName) {
  ...
  // Check if this is a FunctionDeclaration at scope level (needs hoisting preservation)
  const needDeclaration = t.isScopable(fun.parent) || t.isExportNamedDeclaration(fun.parent)
  const isFunctionDeclaration = t.isFunctionDeclaration(fun.node) && fun.node.id && needDeclaration

  if (isFunctionDeclaration) {
    // For function declarations, keep the function and use Object.assign to add properties
    const functionId = fun.node.id
    const properties = buildWorkletProperties(t, fun, fileName)

    // Create Object.assign statement
    const objectAssignCall = t.expressionStatement(
    ...
    // Insert Object.assign after the function declaration
    fun.insertAfter(objectAssignCall)
  } else {
    // For other cases (arrow functions, function expressions), use the original IIFE approach
